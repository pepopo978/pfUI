pfUI:RegisterModule("nameplates", "vanilla:tbc", function()
	...
	local function IsNamePlate(frame)
		if frame:GetObjectType() ~= NAMEPLATE_FRAMETYPE then
			return nil
		end
		...
		if not regions.GetObjectType then
			return nil
		end
		if not regions.GetTexture then
			return nil
		end
	...
	local function DisableObject(object)
		...
		if not object.GetObjectType then
			return
		end
	...
	local function HidePlate(unittype, name, fullhp, target)
		...
		if C.nameplates.fullhealth == "1" and not fullhp then
			return nil
		end
		if C.nameplates.target == "1" and target then
			return nil
		end
		...
		elseif C.nameplates.critters == "1" and unittype == "NEUTRAL_NPC" then
			for i, critter in pairs(L["critters"]) do
				if string.lower(name) == string.lower(critter) then
					return true
				end
			end
	...
	local function PlateUnitDebuff(self, id)
		...
		if not self.debuffcache[id] then
			return
		end
		if not self.debuffcache[id].stop then
			return
		end
	...
	local function UpdateDebuffConfig(nameplate, i)
		if not nameplate.debuffs[i] then
			return
		end
	...
	nameplates.OnDataChanged = function(self, plate)
		...
		local guid = string.lower(plate.guid)
		...
		if target and C.nameplates.cpdisplay == "1" then
			for i = 1, GetComboPoints("target") do
				plate.combopoints[i]:Show()
			end
		end
		...
		if C.nameplates["showdebuffs"] == "1" then
			...
			if C.nameplates["guessdebuffs"] == "1" then
				plate:CacheDebuffs(guid, verify)
			end

			-- check if 3 seconds since last scan
			if not plate.lastscan or plate.lastscan < GetTime() - 1 then
	...
	if pfUI.client <= 11200 then
		...
		nameplates.mouselook:SetScript("OnUpdate", function()
			...
			if not IsMouselooking() then
				...
				if UnitCanAttack("player", "target") and not nameplates.combat.inCombat then
					AttackTarget()
				end
				this:Hide()
				return
